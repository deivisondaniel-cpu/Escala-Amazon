...
# 2. INJEÇÃO DE JAVASCRIPT MASTER
components.html("""
    <script>
...
def inicializar_banco():
    if not os.path.exists(ARQUIVO_BANCO):
        # ⚠️ COMENTÁRIO: Se você precisar mudar o horário inicial padrão de algum funcionário fixo, mude diretamente no texto abaixo (Ex: "15:00 às 19:00" para o seu novo horário)
        operadores_padrao = [
            # Turno 1
    ...
    if not st.session_state.autenticado:
        ...
        if st.button("Adicionar ao Sistema", use_container_width=True):
            if novo_nome and nova_funcao:
                ...
                # ⚠️ COMENTÁRIO: Aqui define o horário padrão quando você CADASTRAR alguém novo pelo painel lateral.
        ...
        for _, row in df_turno.iterrows():
            ...
            for idx, dia in enumerate(dias_lista, 2):
                ...
                if st.session_state.autenticado:
                    # ⚠️ COMENTÁRIO: Aqui embaixo é onde o botão define para qual horário voltar quando você tira alguém da FOLGA.
                    # Se o Turno 1 for outro horário, mude o primeiro. Se o Turno 2 ou 3 for diferente de "15:00 às 19:00", altere o segundo.
